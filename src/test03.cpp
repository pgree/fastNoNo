...
void mixed_2group(int nnt, int nn, int n, int k1, int k2, int k,
		  ...
		  Eigen::VectorXd &dsums, double &dsum, Eigen::VectorXd &stds,
		  Eigen::MatrixXd &cov);
...
// [[Rcpp::export]]
void mixed_2group(int nnt, int nn, int n, int k1, int k2, int k, Eigen::MatrixXd a, Eigen::VectorXd y, Eigen::VectorXd ss, double sigy, double sig1,		  Eigen::VectorXd &dsums, double &dsum, Eigen::VectorXd &stds,
		  Eigen::MatrixXd &dsums_cov) {
...
void eval_inner(int nn, int n, int k1, int k2, int k, double d1,
  ...
  Eigen::MatrixXd vt(k, k), v(k, k), c(k, k), ct(k, k), tmp_mat(k, k),
    asca(k, k);
  ...
  Eigen::JacobiSVD<Eigen::MatrixXd> svd(asca, Eigen::ComputeThinU | Eigen::ComputeThinV);
  ...
  //std::cout << "dsum_vars: " << dsum_vars << std::endl;
...
void get_phi(int nn, int n, int k, double t, Eigen::VectorXd ysmall,
  ...
  double phi, alpha, beta, prefact, exp_fact, a1, sig22, sig12, rho,
    djac, f, fi;
  Eigen::VectorXd fs(nn), tmp_mat(nn);
  Eigen::VectorXd phis(nn), whts_phis(nn);
...
void get_int_bds(int nn, Eigen::VectorXd phis, Eigen::VectorXd fs, int &i0, int &i1) {
  ...
  Eigen::VectorXi tmpvec = ((tmpvec1.array() == 1) && (phis_eig.array() > fmax_phi)) \
    .cast<int>();
  for (int i=0; i<nn; i++) {
    if (tmpvec[i] > 0) {
      i1 = i;
